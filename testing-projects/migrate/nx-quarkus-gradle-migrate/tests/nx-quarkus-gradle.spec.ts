...
describe('@jnxplus/nx-quarkus-gradle migrate', () => {
  beforeEach(async () => {
  ...
  it('should migrate', async () => {
    ...
    execSync('nx generate @jnxplus/nx-quarkus-gradle:init', execSyncOptions());

    execSync(
      `nx g @jnxplus/nx-quarkus-gradle:application ${testApp}`,
      execSyncOptions()
    );

    execSync(
      `nx g @jnxplus/nx-quarkus-gradle:lib ${testLib} --projects ${testApp}`,
      execSyncOptions()
    );

    execSync(
      `nx g @jnxplus/nx-quarkus-gradle:application ${testApp2}`,
      execSyncOptions()
    );

    execSync(
      `nx g @jnxplus/nx-quarkus-gradle:application ${testApp3}`,
      execSyncOptions()
    );

    execSync(
      `nx g @jnxplus/nx-quarkus-gradle:application ${testApp4}`,
      execSyncOptions()
    );

    execSync(
      `nx g @jnxplus/nx-quarkus-gradle:lib ${testLib2} --projects ${testApp2},${testApp3},${testApp4}`,
      execSyncOptions()
    );
    ...
    execSync('npx@next nx migrate --run-migrations --ifExists', execSyncOptions());

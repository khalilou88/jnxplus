...
describe('nx-gradle gradle-root-directory kotlin dsl e2e', () => {
  ...
  beforeAll(async () => {
    ...
    await runNxCommandAsync(
      `generate @jnxplus/nx-gradle:init --dsl kotlin --rootProjectName ${rootProjectName} --gradleRootDirectory deep/subdir --versionManagement properties`,
    );
  }, 120000);
  ...
  it('should use dsl option when initiating the workspace', async () => {
    ...
    expect(() =>
      checkFilesExist(
        'deep/subdir/gradle/wrapper/gradle-wrapper.jar',
        'deep/subdir/gradle/wrapper/gradle-wrapper.properties',
        'deep/subdir/gradlew',
        'deep/subdir/gradlew.bat',
        'deep/subdir/gradle.properties',
        'deep/subdir/settings.gradle.kts',
      ),
    ).not.toThrow();
  }, 120000);

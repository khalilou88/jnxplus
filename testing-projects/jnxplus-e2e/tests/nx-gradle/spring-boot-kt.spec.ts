...
describe('nx-gradle spring-boot kotlin dsl e2e', () => {
  ...
  beforeAll(async () => {
    ...
    await runNxCommandAsync(
      `generate @jnxplus/nx-gradle:init --gradleRootDirectory . --dsl kotlin --rootProjectName ${rootProjectName} --preset none`,
    );
  }, 120000);
  ...
  it('should create a sb java application', async () => {
    ...
    expect(
      rootBuildGradle.includes('libs.plugins.springframework.boot'),
    ).toBeTruthy();

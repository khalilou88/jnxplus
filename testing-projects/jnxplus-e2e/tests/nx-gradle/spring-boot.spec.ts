...
describe('nx-gradle spring-boot e2e', () => {
  ...
  beforeAll(async () => {
    ...
    await runNxCommandAsync(
      `generate @jnxplus/nx-gradle:init --rootProjectName ${rootProjectName} --preset spring-boot --versionManagement properties`,
    );
  }, 120000);
  ...
  it('--an app with aliases', async () => {
    ...
    await runNxCommandAsync(
      `g @jnxplus/nx-gradle:app ${randomName} --framework spring-boot --t e2etag,e2ePackage --dir ${appDir} --groupId com.jnxplus --v 1.2.3 --packaging war --configFormat .yml --port ${port} --simplePackageName false --simpleName false`,
    );
  ...
  it('should generare a lib with a simple package name', async () => {
    ...
    await runNxCommandAsync(
      `generate @jnxplus/nx-gradle:library ${randomName} --framework spring-boot --directory ${libDir} --tags e2etag,e2ePackage --groupId com.jnxplus --simplePackageName --projectVersion 1.2.3 --simpleName false`,
    );
